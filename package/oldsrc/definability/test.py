...
def load_tests(loader, tests, ignore):
    modules = [config, examples, files, fofunctions,
               ...
               morphisms, constellation, lindenbaum, preorder]
...
class Test(unittest.TestCase):
    ...
    def test_open_definable_one_planet(self):
        ...
        tick = datetime.now()
        self.assertEqual(
            c.is_open_definable(tiporet, tiporet + tipoposet), (True, None))
        tock = datetime.now()
        diff = tock - tick    # the result is a datetime.timedelta object
        self.assertEqual(
            (len(c.graph.edges()), len(c.graph.nodes())), (2364, 26))
        ...
        if len(constellations) == 2:
            self.assertEqual(
                constellations[0].is_isomorphic(constellations[1]), True)
